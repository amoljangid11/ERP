...
@Component({
  selector: 'app-overdue1',
  templateUrl: './overdue1.component.html',
  styleUrls: ['./overdue1.component.css'],
  providers:[DatePipe]

})
export class Overdue1Component implements OnInit {
...
t;
  checkk(){
    this.obk=this.allss.filter(obj => obj.allservicereq[0].status != "cancelled" && !obj.allservicereq[0].executorstatus && obj.allservicereq[0].date  )
    
    var Tday1= this.today;
    this.t=this.today+15 * 24 * 60 * 60 * 1000;
    console.log(this.t)
    console.log( this.today )
    this.obk=this.allss.filter(obj => obj.allservicereq[0].status != "cancelled" && ((this.datepipe.transform(obj.allservicereq[0].date+15 * 24 * 60 * 60 * 1000,'yyyy-MM-dd'))==( this.datepipe.transform(this.today,'yyyy-MM-dd'))))
