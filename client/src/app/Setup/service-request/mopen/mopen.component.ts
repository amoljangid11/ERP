...
@Component({
  selector: 'app-mopen',
  templateUrl: './mopen.component.html',
  styleUrls: ['./mopen.component.css']
})
export class MopenComponent implements OnInit {
  ...
  checkk(){
    this.management1=this.allss.filter(obj => obj.allservicereq[0].status=='open' )
    this.management2=this.allss1.filter(obj => obj.allservicereq[0].status=='open' )
    this.management3=this.allss2.filter(obj => obj.allservicereq[0].status=='open' )
