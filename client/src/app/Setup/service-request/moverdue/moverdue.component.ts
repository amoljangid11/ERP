...
@Component({
  selector: 'app-moverdue',
  templateUrl: './moverdue.component.html',
  styleUrls: ['./moverdue.component.css'],
  providers:[DatePipe]

})
export class MoverdueComponent implements OnInit {
...
t;
  ...
  checkk(){
    ...
    console.log(this.t)
    this.management1=this.allss.filter(obj => obj.allservicereq[0].status != "cancelled" && !obj.allservicereq[0].executorstatus && ((this.datepipe.transform(obj.allservicereq[0].date+15 * 24 * 60 * 60 * 1000,'yyyy-MM-dd'))==( this.datepipe.transform(this.today,'yyyy-MM-dd'))) )
    this.management2=this.allss1.filter(obj => obj.allservicereq[0].status != "cancelled" && !obj.allservicereq[0].executorstatus && ((this.datepipe.transform(obj.allservicereq[0].date+15 * 24 * 60 * 60 * 1000,'yyyy-MM-dd'))==( this.datepipe.transform(this.today,'yyyy-MM-dd'))) )
    this.management3=this.allss2.filter(obj =>!obj.allservicereq[0].executorstatus && ((this.datepipe.transform(obj.allservicereq[0].date+15 * 24 * 60 * 60 * 1000,'yyyy-MM-dd'))==( this.datepipe.transform(this.today,'yyyy-MM-dd'))))
