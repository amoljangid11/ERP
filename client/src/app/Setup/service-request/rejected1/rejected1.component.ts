...
@Component({
  selector: 'app-rejected1',
  templateUrl: './rejected1.component.html',
  styleUrls: ['./rejected1.component.css']
})
export class Rejected1Component implements OnInit {
  ...
  checkk(){
    this.obk=this.allss.filter(obj => obj.allservicereq[0].status != "closed" && obj.allservicereq[0].executorstatus == "Rejected" )
   this.oobj1 = this.allss1.filter(obj => obj.allservicereq[0].managementstatus1 == 'Rejected');
  this.oobj2 = this.allss2.filter(obj =>  obj.allservicereq[0].finalmanagerstatus == "Rejected");
 ...
 console.log(this.oobj2)
  ...
  baseAll1.getList().subscribe(data => {
