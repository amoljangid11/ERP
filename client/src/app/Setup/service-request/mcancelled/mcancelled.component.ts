...
@Component({
  selector: 'app-mcancelled',
  templateUrl: './mcancelled.component.html',
  styleUrls: ['./mcancelled.component.css']
})
export class McancelledComponent implements OnInit {
  ...
  checkk(){
    this.management1=this.allss.filter(obj => obj.allservicereq[0].status=='cancelled' )
    this.management2=this.allss1.filter(obj => obj.allservicereq[0].executorstatus=='cancelled' )
    this.management3=this.allss2.filter(obj => obj.allservicereq[0].executorstatus=='cancelled')
