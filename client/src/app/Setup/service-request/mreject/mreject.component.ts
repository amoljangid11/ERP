...
@Component({
  selector: 'app-mreject',
  templateUrl: './mreject.component.html',
  styleUrls: ['./mreject.component.css']
})
export class MrejectComponent implements OnInit {
  ...
  checkk(){
    this.management1=this.allss.filter(obj => obj.allservicereq[0].management1status=='Rejected' )
    this.management2=this.allss1.filter(obj => obj.allservicereq[0].management1status=='Rejected' )
    this.management3=this.allss2.filter(obj => obj.allservicereq[0].management1status=='Rejected')
