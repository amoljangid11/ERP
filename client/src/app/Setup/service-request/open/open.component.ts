...
objjj;
  fetch(){
this.objjj=this.allss.filter(obj => obj.allservicereq[0].status=='open')
console.log(this.objjj)
console.log(this.objjj.status)
  }
...
console.log(this.catt1)
  ...
  requestcancel(det){
    ...
    console.log(det)
    console.log(det.allservicereq[0].status)
    det.allservicereq[0].canceldate=this.today;
    det.allservicereq[0].status='cancelled'
    det.save().toPromise().then(res=>{
      ...
      this.restangular.all('category').getList()
       ...
       msg.savemsg('Request Cancelled Succesfully','alert alert-info');
      })
      // this.alls={allservicereq:[{}]}; 
      this.ngOnInit();
  }
