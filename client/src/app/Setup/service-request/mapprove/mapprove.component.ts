...
@Component({
  selector: 'app-mapprove',
  templateUrl: './mapprove.component.html',
  styleUrls: ['./mapprove.component.css']
})
export class MapproveComponent implements OnInit {
  ...
  checkk(){
    this.management1=this.allss.filter(obj => obj.allservicereq[0].management1status=='Approved' )
    this.management2=this.allss1.filter(obj => obj.allservicereq[0].management1status=='Approved' )
    this.management3=this.allss2.filter(obj => obj.allservicereq[0].management1status=='Approved')
