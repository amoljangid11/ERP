...
updateproj(obj)
{
  obj.save().toPromise().then(function(resp){
        console.log(resp)
  })
    this.updated=true;
    this.messageClass='alert alert-info ';
...
this.pro=data;
// this.findproject(this.pro)
return this.pro;
});

let basecli = this.restangular.all('client');
...
this.proresp=data;
// this.findproject(this.proresp)
console.log(this.proresp);
// var obj = this.proresp.filter(obj=>obj.id==this.getuser.employeeOId)
return this.proresp;
});
